...
export default function Blog2() {
	...
	const fetchDevToPosts = async () => {
		try {
			...
			const response = await fetch('https://dev.to/api/articles?per_page=3&top=7&username=titon', {
				headers: {
	...
	const getReadingTime = (bodyMarkdown) => {
	...
	const getCategoryTag = (tags) => {
		...
		return foundTag ? foundTag.charAt(0).toUpperCase() + foundTag.slice(1) : tags[0].charAt(0).toUpperCase() + tags[0].slice(1)
	}

	// Fallback images in case dev.to doesn't have a cover image
									...
									<div className="blog-card rounded-top-2 mb-lg-3 mb-md-5 mb-3">
										<div className="blog-card__image position-relative">
											<div className="zoom-img rounded-2 overflow-hidden">
												<img 
													className="w-100" 
													src={blog.cover_image || fallbackImages[index]} 
													alt={blog.title}
